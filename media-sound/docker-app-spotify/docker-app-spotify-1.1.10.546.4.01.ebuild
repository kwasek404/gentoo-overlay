...
src_install() {
	docker build -t ${PN}:${PV} ${WORKDIR}/spotify || die "docker build failed"
	cp -f execute.sh ${D}/usr/bin/${PN}
	docker run --rm --entrypoint cat ${PN}:latest /usr/share/icons/hicolor/512x512/apps/spotify.png > ${WORKDIR}/${PN}.png
